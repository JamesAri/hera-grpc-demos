// Caller - calling various gRPC services
...
// various service clients for demo purposes
const ChatClient = require('./chat/client')
const config = require('./config')
const sendFile = require('./file-share/client')
const TestClientInterceptor = require('./interceptors/client-interceptor')
		...
		healthService.check({ service: '' }, (error, response) => {
			if (error) {
				...
				const chat = new ChatClient(stub, async () => {
					stub.close()
					await sc.close()
					console.log('OK - Closed connection after await')
				})
				chat.start()
			}
		})
